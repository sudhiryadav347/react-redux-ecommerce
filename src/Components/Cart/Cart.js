						...
						<tbody>
							{
								cartItems.map((product, index) => {
									...
									const reduceItemHandler = () => {
										dispatch(cartActions.reduceItemFromCart(product.id));
									}
									...
									return (
										<tr key={product.id}>
											<th scope='row'>
												<Image src={product.image} className={styles.thumbnail}></Image>
											</th>
											...
											<td className='align-middle'>${(product.totalPrice).toFixed(2)}</td>
											<td className='align-middle'>
												<Nav.Link
